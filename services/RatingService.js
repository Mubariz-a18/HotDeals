...
module.exports = class RatingService {
  ...
  static async getRating(bodyData, userId) {
    ...
    const RatingDoc = await Rating.findOne({ user_id: bodyData.user_id },
      {
        _id: 0,
        "RatingInfo": {
          $elemMatch: {
    ...
    if (!RatingDoc || !RatingDoc.RatingInfo[0] || !RatingDoc.RatingInfo[0].rating) {
      throw ({ status: 404, message: 'RATING_NOT_EXISTS' });
    }
    return RatingDoc.RatingInfo[0].rating
  };
};
